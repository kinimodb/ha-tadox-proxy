...
class TadoxProxyThermostat(ClimateEntity, RestoreEntity):
    ...
    def __init__(self, hass: HomeAssistant, entry: Any, coordinator: Any) -> None:
        ...
        self._window_open_delay_min: float = float(options.get(CONF_WINDOW_OPEN_DELAY_MIN, 0))
        self._window_close_delay_min: float = float(options.get(CONF_WINDOW_CLOSE_DELAY_MIN, 0))
        ...
        self._hybrid_cfg = HybridConfig(
            ...
            coast_min_setpoint_c=float(options.get("coast_min_setpoint_c", DEFAULT_MIN_TEMP_C)),
            coast_max_setpoint_c=float(options.get("coast_max_setpoint_c", DEFAULT_MAX_TEMP_C)),
        ...
        self._policy = CommandPolicy(
            ...
            fast_recovery_max_c=float(options.get("fast_recovery_max_c", 2.0)),
    ...
    async def _async_restore_state(self) -> None:
        ...
        if "temperature" in last_state.attributes:
            try:
                ...
                self._target_temperature = DEFAULT_TARGET_TEMP_C
