> **[English version](README.md)**  |  Deutsch (diese Seite)
...
> Für Details zur Feinjustierung siehe [TUNING_DE.md](TUNING_DE.md).
...
|----------|-------------|
| `regulation_reason` | Grund der letzten Regelungsentscheidung (z.B. `sent(normal_update)`, `rate_limited(120s)`) |
| `tado_internal_temp_c` | Tado-interne Temperaturmessung (°C) |
| `correction_kp` | Aktiver Proportional-Gain (Kp) |
| `correction_ki` | Aktiver Integral-Gain (Ki) |
| `effective_setpoint_c` | Effektiver Sollwert inkl. Preset (°C) |
...
### Bedingt sichtbar (Sensor-Resilienz)

| Attribut | Beschreibung |
|----------|-------------|
| `room_temp_last_valid_c` | Letzter gültiger Messwert des externen Sensors (°C) |
| `room_temp_last_valid_age_s` | Alter des letzten gültigen Messwerts (Sekunden) |

### Bedingt sichtbar (nach erstem Regelzyklus)

| Attribut | Beschreibung |
|----------|-------------|
| `feedforward_offset_c` | Feedforward-Korrekturoffset (°C) |
| `p_correction_c` | Proportionale (P) Korrektur (°C) |
| `i_correction_c` | Integrale (I) Korrektur (°C) |
| `error_c` | Aktueller Fehler zwischen Soll- und Raumtemperatur (°C) |
| `target_for_tado_c` | Berechneter Sollwert, der an Tado gesendet wird (°C) |
| `is_saturated` | Controller-Sättigung aktiv (`true`/`false`) |
...
| Datei | Zweck |
|-------|-------|
| [TUNING_DE.md](TUNING_DE.md) | Detaillierte Tuning-Anleitung für neue Räume (Deutsch) |
| [TUNING.md](TUNING.md) | Tuning guide for new rooms (English) |
